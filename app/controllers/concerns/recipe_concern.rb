module RecipeConcern
  ...
  def picture_upload(recipe, picture)
  ...
  def picture_update(recipe, picture)
    blob = ActiveStorage::Blob.create_and_upload!(
      io: StringIO.new((Base64.decode64(recipe_params[:picture].split(",")[1]))),
      filename: "#{@recipe.id}_#{Time.current.to_i}.jpeg",
      content_type: "image/jpeg"
  ...
  def user_requesting_own_recipe_resource(resource)
    return @current_user.id == resource.user_id if resource.class == Recipe
    return @current_user.id == resource.recipe.user.id
  end
end